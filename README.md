...
### Definitions
	- Service
		- Service is the mobile application operated by PreDiagnose
	- Personal Data
...
Each chip that we provide for our hardware, belongs to a set: a box of chips that we sell for a specific measurement, and we have a variety of such boxes. Each chip has a QR code on it, as mentioned earlier. The QR code is a sequential UUID that is stored on our server as part of a blob containing all the UUIDs for all the chips for the current box. This is to ensure that 
